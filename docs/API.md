...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `requestNo` | String | 采购申请编号 |
| `purchaseAmount` | double | 采购金额 |
| `supplierRiskScore` | int | 供应商风险分 |
| `priceVarianceRate` | double | 价格偏离率(%) |
| `singleSource` | boolean | 单一来源采购 |
| `sanctionsClear` | boolean | 制裁筛查通过 |
| `beneficialOwnerVerified` | boolean | 受益所有人已核验 |
| `conflictDeclared` | boolean | 利益冲突已声明 |
| `budgetAvailable` | boolean | 预算充足 |
