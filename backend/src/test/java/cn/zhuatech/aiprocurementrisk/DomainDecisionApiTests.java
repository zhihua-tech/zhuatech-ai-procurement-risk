...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"requestNo\":\"PR-2026-0088\",\"purchaseAmount\":800000,\"supplierRiskScore\":18,\"priceVarianceRate\":3,\"singleSource\":false,\"sanctionsClear\":true,\"beneficialOwnerVerified\":true,\"conflictDeclared\":true,\"budgetAvailable\":true}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"requestNo\":\"PR-2026-0088\",\"purchaseAmount\":5000000,\"supplierRiskScore\":90,\"priceVarianceRate\":80,\"singleSource\":true,\"sanctionsClear\":false,\"beneficialOwnerVerified\":false,\"conflictDeclared\":false,\"budgetAvailable\":false}"))
