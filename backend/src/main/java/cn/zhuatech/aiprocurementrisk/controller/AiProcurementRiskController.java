...
import cn.zhuatech.aiprocurementrisk.model.*;
import cn.zhuatech.aiprocurementrisk.service.AiProcurementRiskService;
...
public class AiProcurementRiskController {
    private final AiProcurementRiskService service; public AiProcurementRiskController(AiProcurementRiskService service){this.service=service;}
    @GetMapping("/public/about") ApiResponse<Map<String,Object>> about(){return ApiResponse.ok(service.about());}
    @GetMapping("/catalog") ApiResponse<AiProcurementRiskService.CatalogView> catalog(){return ApiResponse.ok(service.catalog());}
    @GetMapping("/dashboard") ApiResponse<AiProcurementRiskService.Dashboard> dashboard(){return ApiResponse.ok(service.dashboard());}
    ...
    @PostMapping("/records/{id}/comments") ApiResponse<BusinessRecord> comment(@PathVariable Long id,@Valid @RequestBody AiProcurementRiskService.CommentRequest request){return ApiResponse.ok(service.comment(id,request));}
    @GetMapping("/sla-summary") ApiResponse<AiProcurementRiskService.SlaSummary> sla(){return ApiResponse.ok(service.slaSummary());}
    @PostMapping("/records") ApiResponse<BusinessRecord> create(@Valid @RequestBody AiProcurementRiskService.RecordRequest request){return ApiResponse.ok(service.create(request));}
    @PutMapping("/records/{id}") ApiResponse<BusinessRecord> update(@PathVariable Long id,@Valid @RequestBody AiProcurementRiskService.RecordRequest request){return ApiResponse.ok(service.update(id,request));}
    @PostMapping("/records/{id}/actions") ApiResponse<BusinessRecord> action(@PathVariable Long id,@Valid @RequestBody AiProcurementRiskService.ActionRequest request){return ApiResponse.ok(service.action(id,request));}
